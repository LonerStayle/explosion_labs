...
def create_embeddings(docs: List[Dict], model="text-embedding-3-small") -> np.ndarray:
    ...
    if "reactant1" in first_doc:
        ...
        for d in docs:
            # ai_service.py와 동일한 형식으로 맞춤: scenario, material_a, material_b만 사용
            text = (
        ...
        raise ValueError(
            "알 수 없는 문서 형식입니다. 'reactant1' 또는 'scenario' 필드를 확인해주세요."
        )
...
def process_and_save_db(source_path: str, output_index_path: str, output_pkl_path: str):
    ...
    print(
        f"FAISS 인덱스를 '{output_index_path}' 파일로 저장했습니다. (총 {index.ntotal}개 벡터)"
    )
