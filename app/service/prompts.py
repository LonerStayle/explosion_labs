...
2. [참고 자료]{context}에 있는 `scenario`, `scenario_answer`, `result_1`, `result_2`, `result_state_detail`, `gold_melt`, `result_state`, `player_state`, `comment`의 value를를 '절대 변경하지 말고 그대로' 출력 JSON에 채워 넣어라.
2. `result_1`은 화학 반응으로 생성된 주요 물질(없으면 null 대신 빈 문자열).
3. `result_2`는 보조 생성물(없으면 null).
4. 모든 Enum 값은 정확히 정의된 문자열만 사용한다.
5. 절대 스스로 값을 계산하거나 창작해서는 안 된다.
...
[HINT CASES - RULE]
현재 시나리오가 USE_KCN, USE_HNO3HCL 와 같이 사용 목적일 경우는
...
- FREE_HINT: 일반 탐색 또는 USE_* → 물건사용/범주 힌트 1문장.
