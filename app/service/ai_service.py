...
class AiService:
    def __init__(self):
        ...
        for d in raw_help_docs:
            document_text = (
                f"{d.get('scenario')} | {d.get('material_a')} | {d.get('material_b')}"
            )
            combination_documents.append(
                Document(page_content=document_text, metadata=d)
            )

        help_chat_docstore = InMemoryDocstore(
            {str(i): doc for i, doc in enumerate(combination_documents)}
        )
        ...
        combination_docstore = InMemoryDocstore(
            {str(i): doc for i, doc in enumerate(combination_documents)}
        )
        ...
        self.combination_retriever = self.combination_vectordb.as_retriever(
            search_kwargs={"k": 1}
        )
        ...
        self.nano_llm = ChatOpenAI(model="gpt-4.1-nano")
    ...
    async def combination_message(self, req: CombinationReq):
        query = (
            f"{req.scenario.value} | {req.material_a.value} | {req.material_b.value}"
        )
        ...
        # result_state는 원본 데이터에 이미 정확히 설정되어 있으므로
        # 재계산하지 않고 그대로 사용 (또는 result_1, result_2 모두 체크)
        if (
            resp.result_1 == resp.scenario_answer
    ...
    async def help_message(self, req: HelpChatReq):
        hint_case = compute_hint_case_advanced(
            req.scenario, req.select_material, req.question
        )
        if not is_chemical_question(req.question):
            ...
            return resp.content
        is_use_scenario = (req.scenario == GameScenario.USE_HNO3HCL) or (
            req.scenario == GameScenario.USE_HNO3HCL
        )
        if is_use_scenario:
            ...
            D, I = self.help_vectordb.index.search(
                query_emb, len(self.help_chat_documents)
            )
